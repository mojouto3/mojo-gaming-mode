...
- **DNS switcher**: Cloudflare, Quad9, and OpenDNS tweaks in the Network section, each switching IPv4 + IPv6 DNS on every active adapter while gaming mode is on and restoring your original DNS on deactivate. The three are mutually exclusive (picking one turns off the others), acting as a single DNS switcher rather than 3 independent toggles. Cloudflare is in the Esports preset by default.
- **Ultimate Performance power plan**: a standalone Enable/Disable button (not tied to gaming mode or any preset) in the Tweaks tab that unlocks Windows' hidden Ultimate Performance power plan. Shows a warning if the PC has a battery, since this plan can prevent the CPU/GPU from downclocking while unplugged - not recommended for laptops running on battery.
